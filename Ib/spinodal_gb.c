	...
	for(INDEX = 0; INDEX <=  10000; ++INDEX)
	{
		...
		for(i1=0; i1<n_x; ++i1)
		{
			for(i2=0; i2<n_y; ++i2)
			{
			  ...
			  __real__ g[i2+n_y*i1] = 2*A*creal*(1-creal)*(1-creal) - 2*A*creal*creal*(1 - creal) + (creal - 5*creal*(1-creal)*(1-creal) + 5*creal*creal*(1-creal))*(0.25 + (-(n1real*n1real)/2 + (n1real*n1real*n1real*n1real)/2 - (n2real*n2real)/2 + (n2real*n2real*n2real*n2real)/2) + e*n1real*n1real*n2real*n2real);
			  __imag__ g[i2+n_y*i1] = 0.0;

			  __real__ h1[i2+n_y*i1] = (1 + 0.5*creal*creal - 2.5*creal*creal*(1-creal)*(1-creal))*(-n1real + 2*n1real*n1real*n1real + 2*e*n1real*n2real);
			  __imag__ h1[i2+n_y*i1] = 0.0;

			  __real__ h2[i2+n_y*i1] = (1 + 0.5*creal*creal - 2.5*creal*creal*(1-creal)*(1-creal))*(-n2real + 2*n2real*n2real*n2real + 2*e*n2real*n1real);
